...
namespace ECSCoreBenchmarks
{
	...
	public class VectorReflectBenchmarks
	{
		...
		public float BaselineReflect()
		{
			for (int i = 0; i < numVectors; i++)
			{
				vec1[i] = vec1[i].Reflect(vec2[i]);
			}
		...
		[Benchmark]
		public float SystemNumericsReflect()
		{
			for (int i = 0; i < numVectors; i++)
			{
				systemVec1[i] = System.Numerics.Vector3.Reflect(systemVec1[i], systemVec2[i]);
			}
		...
		[Benchmark]
		public float SseReflect()
		{
			ECSCore.Numerics.Vector3Sse.Reflect(vec1, vec2, vec1);

			return vec1[0].x;
		}

		[Benchmark]
		public float AvxReflect()
		{
			ECSCore.Numerics.Vector3Avx.Reflect(vec1, vec2, vec1);
