...
namespace Core.Graphics.VulkanBackend.Utility
{
    public unsafe class NativeList<T> : IEnumerable<T>, IDisposable where T : unmanaged
    {
		...
		public Span<T> Span => new Span<T>(_dataPtr, (int)_count);
		public ReadOnlySpan<T> ReadOnlySpan => new ReadOnlySpan<T>(_dataPtr, (int)_count);

#if DEBUG
		private string allocStackFrame;
#endif
        ...
        public NativeList(uint capacity)
        {
            Allocate(capacity);
#if DEBUG
            allocStackFrame = new StackTrace().ToString();
#endif
        }
        ...
        ~NativeList()
        {
            if (_dataPtr != null)
            {
                Console.WriteLine($"A NativeList<{typeof(T).Name}> was not properly disposed.\n" + allocStackFrame);
                Dispose();
            }
        }
#endif
